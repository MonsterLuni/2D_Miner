...
public class Map {
    ...
    public Map(UI ui) {
        ...
        ui.blocks = new ArrayList<>((worldWidth / 25) * (ui.screenHeight / 25));
        try {
    ...
    public int getBlockFromPlayerX(int X, int Y, boolean right){
        for (int i = 0; i < ui.blocks.size(); i++){
            if(getOnlyVisibleBlocks(i)){
                if(ui.blocks.get(i).X == X && ui.blocks.get(i).Y == Y){
                    if(right){
    ...
    public boolean getOnlyVisibleBlocks(int index){
        ...
        return ui.blocks.get(index).X / 25 >= positionPlayer && ui.blocks.get(index).X / 25 <= positionPlayer + 59;
    }
